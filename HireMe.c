...
void createLevel3Input(u8 target[32], u8 input[32])
{
    //these are the hard-coded locations of 0x00; you could scan for them if faced with a different confusion array
    u8 zero1 = 106;
    u8 zero2 =309-256; //53
    //iterate through each character in <target>
    for (int i = 0; i < 16; i++)
    {
        //iterate through each element of confusion looking for an occurence of that character
        for (int j = 0; j < 512; j++)
        {
            //if we find a match, create a <character, nullcharacter> pair entry in one of the inputs.
            if (target[i] == confusion[j])
            {
                if (j >= 256)
                {
                    //make j the second of the pair
                    input[i * 2] = zero1;
                    input[i * 2 + 1] = (u8)(j - 256);
                }
                else
                {
                    //make j the first of the pair
                    input[i * 2] = (u8)j;
                    input[i * 2 + 1] = zero2;
                }
                break;
